...
# change the variables in mine-user.sh instead #
#################################################

SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )


# If run from macOS, load defaults from mine-macos-env.sh
if [[ "$OSTYPE" == "darwin"* ]]; then
    if [[ -f "$SCRIPT_DIR"/mine-macos-env.sh ]]
        then
        source "$SCRIPT_DIR"/mine-macos-env.sh
    fi
fi

# Read variables from mine-user.sh
# shellcheck source=/dev/null
if [[ -f "$SCRIPT_DIR"/mine-user.sh ]]
then
    source "$SCRIPT_DIR"/mine-user.sh
fi
...
# Name of the subdirectory (defaults to craft-bedrock-mine)
if [[ -z "${clone_dir}" ]]
then
    clone_dir="craft-bedrock-mine"
fi
...
# read any command line flags to the mine.sh script
...
printf "\n%s\n" "${delimiter}"
printf "\e[1m\e[32mInstall script for craft-bedrock + Web UI\n"
...
then
    ...
    printf "\n%s\n" "${delimiter}"
    printf "Clone craft-bedrock-mine"
    printf "\n%s\n" "${delimiter}"
    "${GIT}" clone https://github.com/AUTOMATIC1111/craft-bedrock-mine.git "${clone_dir}"
    cd "${clone_dir}"/ || { printf "\e[1m\e[31mERROR: Can't cd to %s/%s/, aborting...\e[0m" "${install_dir}" "${clone_dir}"; exit 1; }
fi
...
KEEP_GOING=1
export MC_MINE_RESTART=tmp/restart
